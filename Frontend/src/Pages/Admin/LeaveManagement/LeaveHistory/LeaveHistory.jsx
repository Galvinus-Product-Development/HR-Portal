...
export default function LeaveHistory() {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const employeeMap = {};
      data.forEach((employee) => {
        employeeMap[employee.id] = employee; //-----------CHANGED HERE --------//
      });
      setEmployees(employeeMap);
    } catch (err) {
      console.error("Error fetching employees:", err);
  ...
  const fetchLeaveHistory = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch leave history: ${response.statusText}`
        );
      }
      const data = await response.json();
  ...
  const handleDocumentDownload = async (recordId, documentName) => {
    try {
      const url = `/api/leave-history/${recordId}/documents/${encodeURIComponent(
        documentName
      )}`;
      window.open(url, "_blank");
    } catch (err) {
  ...
  const filteredHistory = leaveHistory //---------Changed here --------//
    .filter((record) => {
