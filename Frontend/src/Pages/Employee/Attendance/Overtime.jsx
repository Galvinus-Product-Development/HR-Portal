  ...
  const fetchOvertimeRequests = async () => {
    try {
      const response = await fetch(
        `http://localhost:5003/at/api/overtime/getOvertimeById/${employeeId}`,
        {
          method: "GET",
          headers: {
      ...
      <div className="overtime-table-container">
        <table className="overtime-table">
          <thead className="overtime-table-head">
            <tr>
              <th className="overtime-table-header">Employee Name</th>
              <th className="overtime-table-header">Department</th>
              <th className="overtime-table-header">Applied On</th>
              <th className="overtime-table-header">Overtime Duration</th>
          ...
          <tbody className="overtime-table-body">
            {overtime
              ...
              .map((request) => (
                <tr key={request.id} className="overtime-table-row">
                    ...
                    <div className="overtime-employee-info">
                      ...
                      <div className="overtime-employee-details">
                        <div className="overtime-employee-name">
                          {request?.employee?.personalDetails?.name || "N/A"}
                        </div>
                  ...
                  <td className="overtime-table-cell">
                    {request?.employee?.employmentDetails?.department || "N/A"}
                  </td>
                  <td className="overtime-table-cell">
        ...
        <div className="overtime-modal-overlay">
          <div className="overtime-modal">
            <div className="overtime-modal-header">
              <h2 className="overtime-modal-title">Overtime Details</h2>
              <button
            ...
            <div className="overtime-modal-body">
              <p>
                ...
                {employeeMap.find(
                  (emp) => emp.employeeId === selectedRequest.employeeId
                )?.name || "N/A"}
              </p>
              <p>
              ...
              <p>
                <strong>Overtime Start Time:</strong>{" "}
                {selectedRequest.overtimeDuration}
              </p>
              <p>
                <strong>Duration:</strong> {selectedRequest.overtimeReason}
              </p>
              {/* <p>
            ...
            <div className="overtime-modal-footer">
              ...
              <button
                className="overtime-hold-button"
                onClick={() =>
                  handleFinalDecision(selectedRequest.id, "onHold")
                }
              >
                <CheckCircle className="overtime-icon" /> On Hold
              </button>
              <button
