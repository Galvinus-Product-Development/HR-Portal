...
export default function AttendanceDashboard() {
	...
	const fetchLeaveRecords = async () => {
		try {
			...
			const response = await fetch(
				`http://localhost:4000/api/leave-history/employee/${employeeId}/${year}/${month}`
			);
			...
			setTimeout(
				() => console.log("Updated leaveRecords:", leaveRecords),
				500
			);
			...
			// const totalAbsentDays = totalPaidLeaves + totalUnpaidLeaves;         // ------------------- CHANGED HERE ----------------------
	...
	useEffect(() => {
		console.log(
			"useEffect triggered - Updated leaveRecords:",
			leaveRecords
		);
	}, [leaveRecords]);
	...
	const fetchEmployeeData = async () => {
		try {
			...
			console.log("hascabkajbscasbckjabsckbakucbaskcba", data.attendance);
	...
	const generateCalendarDays = () => {
		...
		if (Array.isArray(leaveRecords) && leaveRecords.length > 0) {
			leaveRecords.forEach((lr) => {
				...
				for (
					...
					const isSameMonth =
						leaveDay.getMonth() === month &&
						leaveDay.getFullYear() === year;
		...
		for (let date = 1; date <= lastDay.getDate(); date++) {
			...
			if (record) {
				...
				console.log(isPastDay);
			
				days.push({
					date,
					status: isPastDay ? "" : "", // or keep it empty if needed
					isLate: false,
					checkIn: "N/A",
					checkOut: "N/A",
				});
